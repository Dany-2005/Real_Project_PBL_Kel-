...
<body class="bg-white font-sans text-gray-900 overflow-x-hidden">
    ...
    @php
        ...
        $judulH1   = $landing->judul_h1        ?? 'Solusi Belanja';
        $highlight = $landing->judul_highlight  ?? 'Lengkap';
        $deskripsi = $landing->deskripsi        ?? 'Temukan segala kebutuhan Anda dalam satu tempat. Kami hadir dengan sistem manajemen modern untuk memberikan pelayanan terbaik bagi pelanggan setia Sarana Agro Makmur.';
    ...
    <nav class="flex justify-end items-center px-6 lg:px-16 py-6 max-w-7xl mx-auto">
    ...
    <header class="max-w-7xl mx-auto px-6 lg:px-16 flex flex-col lg:flex-row items-center py-10 lg:py-16 gap-12">
        <div class="lg:w-1/2 space-y-8 text-center lg:text-left">
            <h1 class="text-5xl lg:text-7xl font-black text-gray-900 leading-tight">
                {{ $judulH1 }}
                <span class="text-[#2d6a4f]">{{ $highlight }}</span> & Terpercaya.
            </h1>
                ...
                <div class="swiper mySwiper shadow-2xl border-8 border-white">
                    <div class="swiper-wrapper">
                        @if($slides && $slides->count() > 0)
                            ...
                            @endforeach
                        @else
                            {{-- Slide default jika belum ada foto/video --}}
